...
if [[ -n "$wifi_activated" ]]; then
    ...
    if [[ "$signal_strength" -gt 80 ]]; then
        echo "$HOME/Dotfiles/nwg-panel/icons/network-wireless-signal-excellent-symbolic.svg"
    elif [[ "$signal_strength" -gt 50 ]]; then
        echo "$HOME/Dotfiles/nwg-panel/icons/network-wireless-signal-good-symbolic.svg"
    elif [[ "$signal_strength" -gt 0 ]]; then
        echo "$HOME/Dotfiles/nwg-panel/icons/network-wireless-signal-weak-symbolic.svg"
    else
        echo "$HOME/Dotfiles/nwg-panel/icons/network-wireless-acquiring-symbolic.svg"
    fi
    echo $connection_name
else
    echo "$HOME/Dotfiles/nwg-panel/icons/network-wireless-disabled-symbolic.svg"
    echo "disconnected"
fi
